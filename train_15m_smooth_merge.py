"""15m training — smoothed + merged to 3 classes.

Rolling majority vote (window=7) stabilizes GMM labels, then merges to BULL/RANGE/BEAR.
...
def load_data(csv_path: str, smooth_window: int = 7):
...
def run_pipeline(csv_path, output_dir, mode, n_trials, xgb_params, lgbm_params, smooth_window=7):
    ...
    export_models(xgb_full, lgbm_full, feature_names, output_dir, "15m", REGIME_CLASSES,
                  metadata_extra={"approach": "smooth_merge", "smooth_window": smooth_window, "class_mapping": CLASS_MAP_3})
    return acc, f1


def main(csv_path="training_15m.csv", output_dir="trained_output_15m_smooth_merge",
         mode="static", n_trials=100, smooth_window=7):
    num_class = len(REGIME_CLASSES)

    if mode in ("static", "both"):
        print("\n" + "=" * 60)
        print("15m Smooth+Merge — STATIC hyperparameters")
        print("=" * 60)
    ...
    if mode in ("optuna", "both"):
        print("\n" + "=" * 60)
        print("15m Smooth+Merge — OPTUNA hyperparameters")
        print("=" * 60)
        X, y, feature_names = load_data(csv_path, smooth_window)
        X_train, X_val, X_test, y_train, y_val, y_test = chronological_split(X, y)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="15m training — smooth + merge")
    parser.add_argument("--csv", default="training_15m.csv")
    parser.add_argument("--output", default="output/trained_output_15m_smooth_merge")
