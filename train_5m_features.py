...
Adds derived features to the raw 28-feature set:
  - Cross-feature interactions (slope×adx, slope×vol)
  - Lag features (previous 1-3 bars for key indicators)
  - Rolling stats (5-bar mean/std for slope and adx)
No class balancing — full imbalanced dataset used as-is.
"""
...
# Core features to create lags/rolling stats for
CORE_FEATURES = [
    "slope_z_5m", "adx_z_5m",
...
def export_models(xgb_model, lgbm_model, feature_names, output_dir):
    ...
    json.dump({
        "timeframe": "5m",
        "approach": "engineered_features",
        "regime_classes": REGIME_CLASSES,
        "model_version": f"v1_features_{int(time.time())}",
...
def main(csv_path="training_5m.csv", output_dir="trained_output_5m_features"):
    print("=" * 60)
    print("5m Training — Feature Engineering")
    print("=" * 60)
    ...
    print(f"  5m/ — XGBoost + LightGBM ensemble (engineered features)")
