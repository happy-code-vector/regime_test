"""Training pipeline for training_5m.csv — 5-class with label smoothing.

Keeps all 5 original classes but applies rolling majority-vote smoothing
to reduce label noise at regime boundaries.
"""
...
def load_data(csv_path: str, smooth_window=7):
    """Load CSV and apply label smoothing."""
...
def main(csv_path="training_5m.csv", output_dir="trained_output_5m_smooth",
         mode="static", n_trials=100, smooth_window=7):
    num_class = len(REGIME_CLASSES)

    if mode in ("static", "both"):
        print("\n" + "=" * 60)
        print("5m Smoothed — STATIC hyperparameters")
        print("=" * 60)
    ...
    if mode in ("optuna", "both"):
        print("\n" + "=" * 60)
        print("5m Smoothed — OPTUNA hyperparameters")
        print("=" * 60)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train 5m with label smoothing")
    parser.add_argument("--csv", default="training_5m.csv")
    parser.add_argument("--output", default="output/trained_output_5m_smooth")
