"""Approach 1 ONLY: Label smoothing (8 classes, no merge).

Rolling majority vote stabilizes the noisy GMM labels without
changing the number of classes. Best window=3 from experiments.
"""
...
def smooth_labels(y: pd.Series, window: int = 3) -> pd.Series:
...
def load_data(csv_path: str, smooth_window: int = 3) -> tuple[pd.DataFrame, pd.Series, list[str]]:
    """Load CSV, smooth labels, keep 8 classes."""
...
def evaluate(xgb_model, lgbm_model, X_test, y_test, tag=""):
    ...
    header = "            " + "  ".join([f"{n:>8s}" for n in REGIME_CLASSES])
    print(header)
    for i in range(len(REGIME_CLASSES)):
        row = "  ".join([f"{cm[i,j]:>8d}" for j in range(len(REGIME_CLASSES))])
        print(f"  {REGIME_CLASSES[i]:10s}: {row}")
...
def export_models(xgb_model, lgbm_model, feature_names, output_dir, smooth_window):
    ...
    json.dump({
        "timeframe": "15m",
        "approach": "smooth_only",
        "regime_classes": REGIME_CLASSES,
        "model_version": f"v2_smooth_{int(time.time())}",
        ...
        "class_mapping": "8 classes, smoothed only (no merge)",
...
def main(csv_path="training_15m.csv", output_dir="trained_output_15m_smooth_only", smooth_window=3):
    print("=" * 60)
    print("Approach 1: Smooth ONLY (8 classes, no merge)")
    print("=" * 60)
    ...
    print(f"  15m/ — XGBoost + LightGBM ensemble (8-class, smoothed)")


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Approach 1: Smooth only")
