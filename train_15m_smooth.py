...
Two key preprocessing steps to fix the ~36% accuracy problem:

1. LABEL SMOOTHING — raw GMM labels flip every candle (75% of consecutive
   steps differ). A rolling majority vote stabilizes them.

2. CLASS MERGING — 8 fine-grained classes collapse to 3:
   - BULL  (classes 0, 1, 2)  — slope_z_15m > 0
   - RANGE (class 3)          — slope_z_15m ≈ 0
   - BEAR  (classes 4, 5, 6, 7) — slope_z_15m < 0
"""
...
# 3-class regime labels after merging
REGIME_CLASSES = ["BULL", "RANGE", "BEAR"]

# Original 8+ classes → 3-class mapping
# Based on slope_z_15m direction analysis:
#   0(+0.106), 1(+0.031), 2(+0.033) → BULL (0)
#   3(+0.001)                        → RANGE (1)
#   4(-0.031), 5(-0.034), 6(-0.111), 7(-0.029) → BEAR (2)
...
def smooth_labels(y: pd.Series, window: int = 7) -> pd.Series:
    ...
    GMM assigns each candle independently — no temporal continuity.
    A centered rolling window picks the dominant regime in the
    neighborhood, killing the rapid flip-flopping.
    """
...
def load_data(csv_path: str, smooth_window: int = 7) -> tuple[pd.DataFrame, pd.Series, list[str]]:
    ...
    y_raw = y_raw.clip(upper=7)  # merge 8/9 → 7
...
def export_models(xgb_model, lgbm_model, feature_names, output_dir, smooth_window):
    ...
    json.dump({
        ...
        "model_version": f"v2_{int(time.time())}",
        ...
        "class_mapping": {
            ...
            "description": "8 GMM classes smoothed then merged to 3: BULL(0,1,2), RANGE(3), BEAR(4,5,6,7)",
        },
...
def main(csv_path="training_15m.csv", output_dir="trained_output_15m", smooth_window=7):
    print("=" * 60)
    print("Regime Classification — 15m Training (smoothed + merged)")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--smooth-window", type=int, default=7, help="Rolling window for label smoothing (odd number recommended)")
