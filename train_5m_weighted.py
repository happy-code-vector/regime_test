"""5m training — class weights only.

Uses sklearn's class_weight='balanced' to penalize misclassification
of rare classes (STRONG_UP/DOWN) more heavily during training.
No resampling — the full dataset is used as-is.
"""
...
def export_models(xgb_model, lgbm_model, feature_names, output_dir):
    ...
    json.dump({
        ...
        "model_version": f"v1_weighted_{int(time.time())}",
...
def main(csv_path="training_5m.csv", output_dir="trained_output_5m_weighted"):
    print("=" * 60)
    print("5m Training — Class Weights")
    print("=" * 60)
    ...
    xgb_full, lgbm_full = train_ensemble(X, y, sample_weight=sw_full)
    export_models(xgb_full, lgbm_full, feature_names, output_dir)

    print(f"\n{'=' * 60}")
    print(f"Done. Output: {Path(output_dir).resolve()}\\")
    print(f"  5m/ — XGBoost + LightGBM ensemble (class weights)")


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="5m training — class weights")
    parser.add_argument("--csv", default="training_5m.csv")
    parser.add_argument("--output", default="trained_output_5m_weighted")
