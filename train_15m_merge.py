"""Approach 2 ONLY: Class merge (3 classes, no smoothing).

8 fine-grained GMM classes collapse to 3 based on slope_z_15m direction:
  - BULL  (classes 0, 1, 2)  — slope > 0
  - RANGE (class 3)          — slope ≈ 0
  - BEAR  (classes 4, 5, 6, 7) — slope < 0
"""
...
def load_data(csv_path: str) -> tuple[pd.DataFrame, pd.Series, list[str]]:
    """Load CSV, merge to 3 classes, NO smoothing."""
...
def export_models(xgb_model, lgbm_model, feature_names, output_dir):
    ...
    json.dump({
        ...
        "model_version": f"v2_merge_{int(time.time())}",
        ...
        "class_mapping": {
            ...
            "description": "8 GMM classes merged to 3: BULL(0,1,2), RANGE(3), BEAR(4,5,6,7)",
        },
...
def main(csv_path="training_15m.csv", output_dir="trained_output_15m_merge"):
    print("=" * 60)
    print("Approach 2: Merge ONLY (3 classes, no smoothing)")
    print("=" * 60)
    ...
    print(f"  15m/ — XGBoost + LightGBM ensemble (3-class, raw labels)")


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Approach 2: Merge only")
