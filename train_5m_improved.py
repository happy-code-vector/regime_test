...
Combines:
  1. Stratified classes — balanced class representation
  2. Class weights — additional penalty for rare class misclassification
  3. Feature engineering — interactions, lags, rolling stats
"""
...
def add_engineered_features(X: pd.DataFrame) -> pd.DataFrame:
    ...
    df["slope_x_adx_15m"] = df["slope_z_15m"] * df["adx_z_15m"]
    df["slope_x_vol_5m"] = df["slope_z_5m"] * df["realized_vol_z_5m"]
    df["adx_x_vol_5m"] = df["adx_z_5m"] * df["realized_vol_z_5m"]
    df["slope_diff_5m_15m"] = df["slope_z_5m"] - df["slope_z_15m"]
    df["adx_diff_5m_15m"] = df["adx_z_5m"] - df["adx_z_15m"]

    # 2. Lag features (previous 1, 2, 3 bars)
    for col in CORE_FEATURES:
        for lag in [1, 2, 3]:
            df[f"{col}_lag{lag}"] = df[col].shift(lag)

    # 3. Rolling stats (5-bar window)
    for col in CORE_FEATURES:
...
def load_data(csv_path: str) -> tuple[pd.DataFrame, pd.Series, list[str]]:
    """Load CSV, add engineered features, drop Warmup rows."""
...
def compute_sample_weights(y):
    ...
    for cls, w in sorted(class_weight_map.items()):
        print(f"    {REGIME_CLASSES[cls]:12s}: {w:.3f}")
    return sample_weights


def train_ensemble(X_train, y_train, X_val=None, y_val=None, sample_weight=None):
    """Train XGBoost + LightGBM ensemble with optional sample weights."""
...
def export_models(xgb_model, lgbm_model, feature_names, output_dir):
    ...
    json.dump({
        "timeframe": "5m",
        "approach": "improved_combined",
        "regime_classes": REGIME_CLASSES,
        "model_version": f"v1_improved_{int(time.time())}",
        ...
        "engineered": {
            "interactions": ["slope_x_adx", "slope_x_vol", "adx_x_vol", "slope_diff", "adx_diff"],
            "lags": "1,2,3 bars for 10 core features",
            "rolling": "5-bar mean and std for 10 core features",
        },
...
def main(csv_path="training_5m.csv", output_dir="trained_output_5m_improved"):
    print("=" * 60)
    print("5m Training — All Improvements Combined")
    print("=" * 60)
    ...
    # Step 3: Compute class weights on balanced training set
    print(f"\n  Computing class weights from training set...")
    sw_train = compute_sample_weights(y_train)

    # Step 4: Train with weights
    xgb_model, lgbm_model = train_ensemble(X_train, y_train, X_val, y_val, sample_weight=sw_train)
    evaluate(xgb_model, lgbm_model, X_test, y_test, tag="Test ")

    # Step 5: Retrain on full balanced data for export
    print(f"\n  Retraining on full balanced data for export...")
    sw_full = compute_sample_weights(y_bal)
    xgb_full, lgbm_full = train_ensemble(X_bal, y_bal, sample_weight=sw_full)
    export_models(xgb_full, lgbm_full, feature_names, output_dir)

    print(f"\n{'=' * 60}")
    print(f"Done. Output: {Path(output_dir).resolve()}\\")
    print(f"  5m/ — XGBoost + LightGBM ensemble (all improvements)")


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="5m training — all improvements combined")
