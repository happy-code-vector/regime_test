"""Training pipeline for training_5m.csv — 3-class regime classification (no smoothing).
...
No label smoothing — raw collapsed labels only.
"""
...
def load_data(csv_path: str):
    """Load CSV and collapse to 3 classes."""
...
def main(csv_path="training_5m.csv", output_dir="trained_output_5m_3class_raw",
         mode="static", n_trials=100):
    num_class = len(REGIME_CLASSES)

    if mode in ("static", "both"):
        print("\n" + "=" * 60)
        print("5m 3-Class Raw — STATIC hyperparameters")
        print("=" * 60)
    ...
    if mode in ("optuna", "both"):
        print("\n" + "=" * 60)
        print("5m 3-Class Raw — OPTUNA hyperparameters")
        print("=" * 60)
